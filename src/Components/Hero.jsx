  ...
  return (
    <section id="home" className="relative">
      ...
      <div className="flex flex-col justify-center py-20 md:py-28 lg:py-36 items-center px-4">
        <div className="flex items-center justify-center border backdrop-blur-md text-ternary rounded-full w-20 h-20 md:w-24 md:h-24 mb-6 bg-[rgba(255,255,255,0.05)]">
          ...
          {heroData.links.map((d, i) => {
            return (
              <a key={i} href={`#${d.link.toLowerCase()}`}>
                <button
                  key={i}
                  className="border cursor-pointer bg-[rgba(255,255,255,0.05)] backdrop-blur-md border-ternary text-base sm:text-lg text-primary font-semibold w-full sm:w-40 md:w-52 py-3 rounded-4xl capitalize hover:-translate-y-2 transition-all duration-200 hover:shadow-[0px_10px_20px_0px_#5E936C] hover:border-[#5E936C]"
                >
                  {d.btnName}
                </button>
              </a>
